...
import 'package:paas_dashboard_portal_flutter/api/mongo/mongo_instance_api.dart';
...
class MongoInstanceListViewModel extends ChangeNotifier {
  ...
  Future<void> fetchMongoInstances() async {
    final results = await MongoInstanceApi.mongoInstances();
  ...
  Future<void> createMongo(String name, String addr, String username, String password) async {
    MongoInstanceApi.saveMongo(name, addr, username, password);
    fetchMongoInstances();
  }

  Future<void> deleteMongo(int id) async {
    MongoInstanceApi.deleteMongo(id);
    fetchMongoInstances();
  }
}
